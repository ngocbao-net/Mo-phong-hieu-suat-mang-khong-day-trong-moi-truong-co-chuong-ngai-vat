%% Sweep lambda / building density using PPP obstacles for full traceability
clear; close all;
warning('off','all')
...
B = 10e6; NF_dB = 9; P_tx_dBm = 30; G_ant_dBi = 5; freq_GHz = 2.0;
freq = freq_GHz * 1e9;
...
for idx = 1:length(lambda_list)
    ...
    acc_buf_kNN = zeros(N_run,1); acc_buf_kNMAP = zeros(N_run,1);
    mae_pl_buf_kNN = zeros(N_run,1); mae_pl_buf_kNMAP = zeros(N_run,1);
    mae_spd_buf_kNN = zeros(N_run,1); mae_spd_buf_kNMAP = zeros(N_run,1);
    for i = 1:N_run
        ...
        [PL_true, ~, ~, ~, Speed_true] = compute_downstream_from_labels(true_labels, mesh, ...
            PL_LOS_func, PL_NLOS_func, P_tx_dBm, G_ant_dBi, Noise_dBm, B);
        [PL_knn, ~, ~, ~, Speed_knn] = compute_downstream_from_labels(est_kNN, mesh, ...
            PL_LOS_func, PL_NLOS_func, P_tx_dBm, G_ant_dBi, Noise_dBm, B);
        [PL_knmap, ~, ~, ~, Speed_knmap] = compute_downstream_from_labels(est_kNMAP, mesh, ...
            PL_LOS_func, PL_NLOS_func, P_tx_dBm, G_ant_dBi, Noise_dBm, B);
    ...
    fprintf('lambda = %.5f | Acc kNN = %.2f | Acc kNMAP = %.2f | MAE PL = %.2f/%.2f | MAE Spd = %.2f/%.2f\n', ...
        lambda, acc_kNN(idx), acc_kNMAP(idx), mae_pl_kNN(idx), mae_pl_kNMAP(idx), mae_spd_kNN(idx), mae_spd_kNMAP(idx));
end
...
plot(lambda_list, acc_kNN, 'b-o', 'LineWidth', 2, 'MarkerSize', 8, 'DisplayName', 'kNN');
plot(lambda_list, acc_kNMAP, 'r-s', 'LineWidth', 2, 'MarkerSize', 8, 'DisplayName', 'kNMAP');
xlabel('\lambda'); ylabel('Accuracy (%)'); title('Accuracy vs obstacle density');
grid on; box on; legend('Location','best'); hold off;
export_current_figure(out_dir, '18_accuracy_vs_lambda');

figure('Color','w'); hold on;
plot(lambda_list, mae_pl_kNN, 'b-o', 'LineWidth', 2, 'MarkerSize', 8, 'DisplayName', 'kNN');
plot(lambda_list, mae_pl_kNMAP, 'r-s', 'LineWidth', 2, 'MarkerSize', 8, 'DisplayName', 'kNMAP');
xlabel('\lambda'); ylabel('MAE Path Loss (dB)'); title('Path-loss MAE vs obstacle density');
grid on; box on; legend('Location','best'); hold off;
export_current_figure(out_dir, '19_mae_pathloss_vs_lambda');

figure('Color','w'); hold on;
plot(lambda_list, mae_spd_kNN, 'b-o', 'LineWidth', 2, 'MarkerSize', 8, 'DisplayName', 'kNN');
plot(lambda_list, mae_spd_kNMAP, 'r-s', 'LineWidth', 2, 'MarkerSize', 8, 'DisplayName', 'kNMAP');
xlabel('\lambda'); ylabel('MAE Speed (Mbps)'); title('Speed MAE vs obstacle density');
grid on; box on; legend('Location','best'); hold off;
export_current_figure(out_dir, '20_mae_speed_vs_lambda');
